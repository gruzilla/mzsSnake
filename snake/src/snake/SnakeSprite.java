...
public class SnakeSprite
{
                     ...
                     BackgroundManager gameMap, SnakePanel snakePanel, int collisionMode)
  {
    ...
    skinsManager.setCurrentSkin(player.getSkin());
    ...
    data = new SnakeSpriteData(skinsManager.getClipsLoader(), conn, player, gameMap, collisionMode);
    data.setSprite(this);
    writingSnake = true;
    loadGraphics();
  ...
  /**
   * SnakeSprite constructor for the snake of another player. The snake is set to passive so data is
   ...
   * @param snakePanel SnakePanel where the game is drawn
   */
  public SnakeSprite(ImageLoader imgLoader, CorsoConnection conn, Player aPlayer, SnakePanel snakePanel)
  {
    player = aPlayer;
    this.imgLoader = imgLoader;
    skinsManager.setCurrentSkin(player.getSkin());
    ...
    msgsFont = new Font("SansSerif", Font.BOLD, 16);
    metrics = snakePanel.getFontMetrics(msgsFont);
    data = new SnakeSpriteData(conn, player, skinsManager.getClipsLoader());
    data.setSprite(this);
    writingSnake = false;
    loadGraphics();
    restartSnake(); //start effect for the snake
  ...
  {
    data.setOtherPlayers(otherPlayer);
  }
  ...
  public void drawSprite(Graphics g, java.awt.Point correctionPos)
  {
    ...
    int headPos = data.headPos;
    int tailPos = data.tailPos;
    int maxPos = headPos;
    if (maxPos < tailPos)
    {
      maxPos += data.MAXPOINTS;
    }
    if (headPos > 0)
    {
      headPos -= 1;
    ...
    {
      headPos = data.MAXPOINTS - 1;
    }

    int tempPos = data.pixelHeadPos-((maxPos-tailPos)*(data.PARTDIST/data.POINTDIST));
    if (tempPos < 0)
    ...
    //draw head
    g2d.drawImage(imgLoader.getBrighterImage(imgLoader.getRotatedImage(imgHead, (int) (data.parts[headPos].direction - 90)),effect.hasBrightenEffect() ? effect.getBrightenValue() : 1.0f),
    ...
    g2d.drawString(player.getName(),
                   (int) data.parts[headPos].x + (imgHead.getWidth() / 2) -
                   (metrics.stringWidth(player.getName()) / 2) - correctionPos.x,
                   (int) data.parts[headPos].y - 10 - correctionPos.y);
  ...
  {
    return new Rectangle( (int) data.parts[data.headPos].x + 2, (int) data.parts[data.headPos].y + 2,
  ...
  {
    return data.parts[data.headPos];
  }
