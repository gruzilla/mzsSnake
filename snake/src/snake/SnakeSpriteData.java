...
public class SnakeSpriteData
{
  ...
  //CorsoVarOids for the data of the snake
   ...
   */
  public SnakeSpriteData(ClipsLoader aClipsLoader, CorsoConnection conn, Player aPlayer,
   ...
   * @param aClipsLoader ClipsLoader with the eat, crash and die sounds of the snake
   */
  public SnakeSpriteData(CorsoConnection conn, Player aPlayer, ClipsLoader aClipsLoader)
  {
    myPlayer = aPlayer;
    clipsLoader = aClipsLoader;
    this.conn = conn;
  ...
  /**
   * SnakeSpriteNotifier class is run in an own thread. All changes that occur by notifications
   * are automatically commited to the SnakeSpriteData object. Following notifications can occur:
   * - new snake state: start sound and effect if snake crashed or died
   * - new snake head position: update the pixel positions of the snake
   * - new snake tail position
   * - new coordinates for a snake part: find and update the affected snake part
   */
  private class SnakeSpriteNotifier implements Runnable
  {
    private boolean running = true;

    public SnakeSpriteNotifier()
    {
    }
    ...
    public void run()
    {
      try
      {
        ...
        CorsoNotificationItem notifItem =
            new CorsoNotificationItem(new CorsoVarOid(snakeStateOid),
                                      0,
        ...
        if (gameMap == null)
        {
          ...
          {
            notifItem =
                new CorsoNotificationItem(new CorsoVarOid(snakePosList.oidList[i]),
                                          0,
                                          CorsoNotificationItem.CURRENT_TIMESTAMP);
            notifVec.addElement(notifItem);
          }
          ...
          notifItem =
              new CorsoNotificationItem(new CorsoVarOid(snakeHeadOid),
                                        0,
                                        CorsoNotificationItem.CURRENT_TIMESTAMP);
          notifVec.addElement(notifItem);

          notifItem =
              new CorsoNotificationItem(new CorsoVarOid(snakeTailOid),
                                        0,
                                        CorsoNotificationItem.CURRENT_TIMESTAMP);
          notifVec.addElement(notifItem);
        }
        ...
        while (running)
        {
          ...
          {
            if (fired.varOid() != null)
            {
              int readOK = -5;
              while (readOK < 0) //read multiple times if reading fails
              {
                try
                {
                  if (fired.varOid().equals(snakeStateOid))
                  {
                   ...
                   // System.out.println("SNAKESTATE: " + snakeState);
                    if (snakeState == SnakeState.crashed)
                    {
                    ...
                    else if (snakeState == SnakeState.unverwundbar)
                    {
                  ...
                  else if (fired.varOid().equals(snakeHeadOid))
                  {
                    ...
                    while (headPos != newHeadPos)
                    {
                      ...
                      updatePixelPositions(headPos, prevPos, false);
                    }
                  }
                  else if (fired.varOid().equals(snakeTailOid))
                  {
  ...
  /**
   * Write the state of the snake to corsospace, if in multiplayer mode. Try up
   * to 10 times if writing fails.
   */
    ...
    if (multiplayer)
    {
      ...
      while (count > 0)
      {
        try
        {
          ...
          snakeStateOid.writeInt(snakeState.ordinal(), CorsoConnection.NO_TIMEOUT);
          //tx.commit(CorsoConnection.INFINITE_TIMEOUT);
          count = -1;
        }
        catch (CorsoException ex)
        {
          System.out.println("SnakeSpriteData: writeState: Corso Error occured:");
          ex.printStackTrace(System.out);
          count--;
        }
      }
      if (count == 0)
      {
        System.out.println("SnakeSpriteData can't set writeState!");
      }
    }
  }
  ...
  private void createData(double startX, double startY, int degree)
  {
    ...
    if (multiplayer)
    {
      ...
      while (count > 0) //try multiple times if writing fails
      {
        ...
        {
          System.out.println("createData: Corso Error occured:");
          ex.printStackTrace(System.out);
          count--;
        }
      }
      if (count == 0)
      {
        System.out.println("SnakeSpriteData can't write createData!");
      }
  ...
  private void initPixelPositions(boolean includeSpeedChange)
  {
    ...
    {
      SnakePos pos1 = parts[i];
      SnakePos pos2 = parts[i + 1];
    ...
    pixelPos[pixelHeadPos] = new java.awt.Point( (int) parts[headPos].x, (int) parts[headPos].y);
    pixelDirection[pixelHeadPos] = (int) parts[headPos].direction;
  }
  ...
  {
    SnakeStartPoint start = gameMap.getStartpunkt(myPlayer.getNr());
  ...
  private void restartSnake()
  {
    ...
    SnakeStartPoint newStartPoint = gameMap.getStartpunkt(myPlayer.getNr());
    starting = STARTPARTS * POINTDIST;
  ...
  private void writeSnakePositions()
  {
    ...
    if (count == 0)
    {
      System.out.println("SnakeSpriteData: can't set writeSnakePositions!");
    }
  }
  ...
  private boolean checkCrash(Rectangle controllRect, int abstandHead, int snakeLength)
  {
    ...
    while (start != stop)
    {
      ...
      Rectangle rect = new Rectangle( (int) parts[start].x + 2, (int) parts[start].y + 2,
                                     snakePartWidth - 4,
                                     snakePartHeight - 4);
    ...
    if (useCollisionOTHER())
    {
      if (otherSnakes != null && otherSnakes.length > 0)
      {
        for (int j = 0; j < otherSnakes.length; j++)
        {
          ...
          Rectangle headRectOther = new Rectangle( (int) otherSnakeData.parts[otherSnakeData.headPos].x + 2,
                                                  (int) otherSnakeData.parts[otherSnakeData.headPos].y + 2,
                                                  snakePartWidth - 4, snakePartHeight - 4);
  ...
  private boolean checkFreePlace(double turn)
  {
    ...
    int controllPosX = (int) (parts[headPos].x - changeX);
    int controllPosY = (int) (parts[headPos].y - changeY);
  ...
  public void updateData(double turn)
  {
    ...
    int prevPos = headPos; // save old head pos while creating new one
    headPos = (headPos + 1) % MAXPOINTS;
    tailPos = (tailPos + 1) % MAXPOINTS;

    double newPosX = parts[prevPos].x - changeX;
    double newPosY = parts[prevPos].y - changeY;
    ...
    if (!setChanges)
    {
      parts[headPos].x = parts[prevPos].x;
      parts[headPos].y = parts[prevPos].y;
    }
    else
    {
      parts[headPos].x = newPosX;
      parts[headPos].y = newPosY;
    }
    parts[headPos].direction = direction;

    //calculate pixel positions from last headpos to new headpos
    updatePixelPositions(headPos, prevPos, true);
  ...
  private void updatePixelPositions(int headPos, int prevHeadPos, boolean activeSnake)
  {
    ...
    if (activeSnake)
    {
      ...
      xIncrement = (parts[headPos].x - parts[prevHeadPos].x) / (double) step;
      yIncrement = (parts[headPos].y - parts[prevHeadPos].y) / (double) step;
    }
    else
    {
      //passive (reading) snake: control if speed has changed, by checking the distance between the two points
      double x = parts[headPos].x - parts[prevHeadPos].x;
      double y = parts[headPos].y - parts[prevHeadPos].y;
    ...
    {
      pixelPos[pixelHeadPos] = new java.awt.Point( (int) (parts[prevHeadPos].x + xIncrement * i),
                                                  (int) (parts[prevHeadPos].y + yIncrement * i));
      pixelDirection[pixelHeadPos] = (int) parts[prevHeadPos].direction;
      pixelHeadPos++;
